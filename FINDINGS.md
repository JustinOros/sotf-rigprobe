...
The player has no emote system. `IPlayerState` has no emote field, but the Bolt event `updateMecanimRemoteState` (layer, state hash, normalizedTime) tells other clients to play a state on a remote player, so any state that already exists in the player controller can be shown to vanilla clients.

Player controller `PlayerAControllerGenerated`: Generic avatar, 12 layers (Base Layer, AdditiveArms, upperBody, UpperBodyReactions, fullBodyActions, leftArm, spineAddititve2, ClientPredictionLayer, Inventory, InventoryUpperBody, Face, Blink), 1205 clips, 271 parameters. Pose and gesture states that exist on the player:

| State | Layer |
|---|---|
| couchIdle, seatedRockIdle | fullBodyActions |
| injuredLoop, standUpIdle, standup, wakeUp | fullBodyActions |
| checkArms, coldIdleVar1 | upperBody |
| PlayerALighterLookAt | Base Layer |
| PlayerAGuitar clips | upperBody |
| PlayerAAngry, PlayerAScared | Face |

Kelvin (`RobbyController`, 86% bone match with the player) plays his emotes as full body states on Base Layer. Captured with `rigwatch robby`:

| State | Clip | Short hash | Full hash |
|---|---|---|---|
| ThumbsUp | ThumbsUp | 1712013045 | -1646602863 |
| ShakeHead | ShakeHead | -1770187688 | 27107153 |
| (unnamed) | RobbyEmoteYes | 1721109824 | 1053072325 |
| IdleScratchHead | IdleScratchHead | -628397148 | 1736991184 |
| IdleSwatFly | IdleSwatFly | -331262521 | 476507385 |
| IdleLookVar1 | IdleLookVar1 | 336950523 | -712749335 |

Other Kelvin emote clips: NodHead, RobbyEmoteHappy, RobbyEmoteSad, RobbyEmoteNo, RobbyEmoteConfused, RobbyPointIntro, RobbyPointLoop, RobbyPointOutro, HitHeadSmall, HitHeadBig, ShakeHeadStepBack. Virginia (`VirginiaController`): LaughWithPlayer, ThumbsUpReaction, PointStanding, VirginiaEmoteHappy, VirginiaEmoteSad, VirginiaEmoteYes, VirginiaEmoteNo, VirginiaEmoteConfused.

Kelvin and Virginia clips are not in the player controller, so playing them on a player (for example with a PlayableGraph clip overriding the player Animator) is only visible to modded clients.
